...
{
   #if(Timer_UsingFixedFunction)
       return ((uint8)CY_GET_REG16(Timer_PERIOD_LSB_PTR));
   #else
       return (CY_GET_REG8(Timer_PERIOD_LSB_PTR));
   #endif /* (Timer_UsingFixedFunction) */
}
...
void Timer_WritePeriod(uint8 period) 
{
    ...
    #else
        CY_SET_REG8(Timer_PERIOD_LSB_PTR, period);
...
{
   #if(Timer_UsingFixedFunction)
       return ((uint8)CY_GET_REG16(Timer_CAPTURE_LSB_PTR));
   #else
       return (CY_GET_REG8(Timer_CAPTURE_LSB_PTR));
   #endif /* (Timer_UsingFixedFunction) */
}
...
void Timer_WriteCounter(uint8 counter) 
{
    ...
    #else
        CY_SET_REG8(Timer_COUNTER_LSB_PTR, counter);
